...
def list_products(
    method, products_filter=None, pathname="website/products"
) -> dict[str, list[dict]]:
    ...
    for product_file in glob(f"{pathname}/*.md"):
        ...
        with open(product_file) as f:
            ...
            if "auto" in data:
                configs = list(
                    filter(lambda config: method in config.keys(), data["auto"])
                )
                if len(configs) > 0:
...
def fetch_urls(
    urls, data=None, headers=None, max_retries=10, backoff_factor=0.5, timeout=30
) -> list[Response]:
    try:
        with FuturesSession() as session:
            ...
            futures = [
                session.get(
                    url, headers=headers, data=data, timeout=timeout, stream=None
                )
                for url in urls
            ]
    ...
    ) as e:  # See https://github.com/psf/requests/issues/4771#issue-354077499
        ...
        if next_max_retries == 0:
            ...
            return fetch_urls(
                urls, data, headers, next_max_retries, backoff_factor, timeout
            )


def fetch_url(
    url, data=None, headers=None, max_retries=5, backoff_factor=0.5, timeout=30
) -> str:
    return fetch_urls([url], data, headers, max_retries, backoff_factor, timeout)[
        0
    ].text
