...
RELNOTES_URL_TEMPLATE = (
    "https://docs.splunk.com/Documentation/Splunk/{version}/ReleaseNotes/MeetSplunk"
)
...
latest_minor_versions_urls = [
    RELNOTES_URL_TEMPLATE.format(version=v) for v in latest_minor_versions
]
...
for response in endoflife.fetch_urls(latest_minor_versions_urls):
    for version, date_str in re.findall(PATTERN, response.text, re.MULTILINE):
        version = (
            f"{version}.0" if len(version.split(".")) == 2 else version
